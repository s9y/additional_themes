...
<div id="rlmenue">
    ...
    {if $template_option.enablekat == 'true'}     <li><a href="#">{$CONST.CATEGORY}</a> <ul> {serendipity_showPlugin class="serendipity_categories_plugin"}</ul></li>{/if}
	{if $template_option.enablearch == 'true'}<li><a href="#">{$CONST.ARCHIVES}</a> {serendipity_showPlugin class="serendipity_archives_plugin"}</li>{/if}
        {if $template_option.enableastat == 'true'} <li><a href="#">           
        {$template_option.statheader} </a><ul>{serendipity_showPlugin class="serendipity_plugin_staticpage"}</ul></li> {/if}			
	{if $template_option.enableuser == 'true'}<li><a href="#">{$template_option.dropheader} </a> <ul>{foreach from=$droplinks item="droplink" name=navbar}<li><a href="{$droplink.href}" title="{$droplink.title}">{$droplink.title}</a></li>{/foreach}</ul></li>{/if}	 						
...
<div id="cr1">	<strong><a style="white-space: nowrap" href="{$plugin_calendar_head.uri_month}/">{$plugin_calendar_head.month_date|formatTime:"%B '%y":false}</a></strong></div><br/>


  
 <div id="leiste"> <ul> <li class="selected">
                        {if $template_option.wppddd == 'true'}  {$template_option.addvtext}      {/if}
                       </li>
                   </ul> 
