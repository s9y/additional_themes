<!-- ENTRIES START -->
    ...
    <div class="serendipity_Entry_Date">
        ...
        <div class="serendipity_entry serendipity_entry_author_{$entry.author|@makeFilename} {if $entry.is_entry_owner}serendipity_entry_author_self{/if}">
            <p class="meta">{if $entry.categories}{$CONST.FILED_UNDER}{foreach from=$entry.categories item="category" name="categories"}<a href="{$category.category_link}">{$category.category_name|@escape}</a>{if not $smarty.foreach.categories.last}, {/if}{/foreach} -{/if} <a href="{$entry.link_author}">{$entry.author}</a> @ {$entry.timestamp|@formatTime:'%H:%M'}{if $entry.is_entry_owner and not $is_preview} - <a href="{$entry.link_edit}">{$CONST.EDIT_ENTRY}</a>{/if}</p>
{if $entry.categories}
